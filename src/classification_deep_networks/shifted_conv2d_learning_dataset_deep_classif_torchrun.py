...
# TODO: Add modulation to compare with more deformable convolutions?
class Conv2dShifted(torch.nn.Module):
    def __init__(self, in_channels, out_channels, k, stride=1, dilation=1,
                 ker_fixed=False, bias=True, eps=1e-6,
                 device='cpu'):
        super(Conv2dShifted, self).__init__()
        ...
        self.deform_conv2d = torchvision.ops.DeformConv2d(
            in_channels, out_channels, k, stride=stride, padding=int((k // 2) * dilation), dilation=dilation, bias=bias,
            groups=1
        )
        self.ker_fixed = ker_fixed
        self.device = device
        self.conv_offsets = torch.nn.Conv2d(
            in_channels, 2, k, stride=stride, padding=int((k // 2)*dilation), dilation=dilation,
        ).to(device)
        self.mask = None
        self.eps = eps
    ...
    def init_params(self):
        if self.ker_fixed:
            # Is still a parameter but will be filtered out in the optimizer
            self.deform_conv2d.weight.requires_grad = False

        # Overwrite some initialisations for fair comparison with our method
        self.deform_conv2d.weight.data.fill_(1 / (self.deform_conv2d.kernel_size[0] ** 2 * self.deform_conv2d.in_channels))
        if self.deform_conv2d.bias is not None:
            self.deform_conv2d.bias.data.fill_(0)

        self.conv_offsets.weight.data.fill_(0 + self.eps)

    def forward(self, inp):
        offsets = self.conv_offsets(inp)
        offsets = offsets.unsqueeze(1).unsqueeze(1)  # (batch, 1, 1, 2, h, w)
        # Repeat offsets to k * k * 2
        offsets = offsets.tile((1, *self.deform_conv2d.kernel_size, 1, 1, 1))  # (batch, k, k, 2, h, w)
        offsets = offsets.reshape((offsets.shape[0], -1, *offsets.shape[-2:]))  # (batch, k*k*2, h, w)
        out = self.deform_conv2d(inp, offsets, self.mask)
        return out


def convert_model_to_shift_conv(model_name, model,
                                 k=None, ker_fixed=False,
                                 eps=1e-6,
                                 no_pooling_or_stride_conv1=False):


    # TODO: add a more parameter to convert add modulation weights of kernels at each pixel locations?
    ...
    # The methodology is the same in our method
    ...
    if model_name in ['ResNet18', 'ResNet34', 'ResNet50', 'ResNet101', 'ResNet152']:
        ...
        layers_changed = [model.layer2, model.layer3, model.layer4]
        ...
        if model_name in ['ResNet18', 'ResNet34']:
            ...
            for layer_idx, layer in enumerate(layers_changed):
                for bottleneck_idx, bottleneck in enumerate(layer):
                    ...
                    bottleneck.conv2 = Conv2dShifted(
...
def main_shifted_conv2d():
    ...
    dir_res = os.path.join(
        ...
        'classification',
        dataset_name,
        'shifted_conv2d_ker_fixed' if ker_fixed else 'shifted_conv2d',
    ...
    model = convert_model_to_shift_conv(
    ...
    plot_results = True
    if plot_results:
        if not os.environ.keys().__contains__("LOCAL_RANK") or (
                os.environ.keys().__contains__("LOCAL_RANK") and dev == 0):
            ...
            delta_loss_normalised = (torch.tensor(loss_tracker_val) - torch.tensor(loss_tracker_train)) / torch.tensor(
                loss_tracker_train)
