...
class Conv2dDeform(torch.nn.Module):
    def __init__(self, in_channels, out_channels, k, stride=1, dilation=1,
        ...
        self.conv_offsets = torch.nn.Conv2d(
            in_channels, k * k * 2, k, stride=stride, padding=int((k // 2)*dilation), dilation=dilation,
        ).to(device)
        self.mask = None
        self.eps = eps
...
def convert_model_to_deform_conv(model_name, model,
...
def main_deform_conv2d():
    ...
    dir_res = os.path.join(
        ...
        'classification',
        dataset_name,
        'deform_conv2d_ker_fixed' if ker_fixed else 'deform_conv2d',
    ...
    model = convert_model_to_deform_conv(
